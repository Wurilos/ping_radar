  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', position: 'relative' }}>
      ...
      <div className="main-content" style={{ flex: 1, marginLeft: 292, minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
        <header style={{ height: 72, margin: '16px 16px 0 0', padding: '0 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, border: '1px solid var(--color-border)', borderRadius: 'var(--radius-xl)', background: 'var(--color-bg-sidebar)', backdropFilter: 'blur(32px)', position: 'sticky', top: 16, zIndex: 40, boxShadow: 'var(--shadow-md)' }}>
          <button onClick={() => setSidebarOpen(true)} style={{ display: 'none', background: 'none', border: 'none', color: 'var(--color-text-primary)', cursor: 'pointer', fontSize: 24, padding: 4 }} className="mobile-menu-btn">☰</button>
          <div className="dashboard-date" style={{ fontSize: 13, color: 'var(--color-text-muted)', fontWeight: 600, letterSpacing: '1px', textTransform: 'uppercase' }}>
            {new Date().toLocaleDateString('pt-BR', { weekday: 'short', year: 'numeric', month: 'long', day: 'numeric' })}
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <ThemeToggle />
            <span className="dashboard-user-name" style={{ fontSize: 14, color: 'var(--color-text-primary)', fontWeight: 600 }}>{user.name}</span>
        ...
        <main style={{ padding: '32px 16px 32px 0', flex: 1, width: '100%' }} className="animate-fade-in">{children}</main>
      </div>
      <style jsx global>{`
        @media (max-width: 768px) {
          ...
          .dashboard-date, .dashboard-user-name { display: none !important; }
        }
      `}</style>
    </div>
  );
